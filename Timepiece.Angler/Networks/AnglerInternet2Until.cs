...
/// </summary>
public class AnglerInternet2Until : UntilNetwork<RouteEnvironment, string>
{
  public AnglerInternet2Until(Digraph<string> digraph,
    ...
    Dictionary<(string, string), Zen<bool>> parentRelation,
    Dictionary<string, Func<Zen<RouteEnvironment>, Zen<bool>>> phiAnnotations,
    Dictionary<string, Func<Zen<RouteEnvironment>, Zen<bool>>> psiAnnotations,
    Dictionary<string, Func<Zen<RouteEnvironment>, Zen<bool>>> livenessProperties,
    ISymbolic[] symbolics) : base(
    digraph, transferFunctions, mergeFunction, initialValues, parentRelation, phiAnnotations, psiAnnotations, livenessProperties,
    symbolics)
  {
  }
  ...
  public AnglerInternet2Until(Digraph<string> digraph,
    ...
    Func<Zen<RouteEnvironment>, Zen<RouteEnvironment>, Zen<RouteEnvironment>> mergeFunction,
    Dictionary<string, Zen<RouteEnvironment>> initialValues,
    Dictionary<string, Zen<BigInteger>> witnessTime,
    Dictionary<string, Func<Zen<RouteEnvironment>, Zen<bool>>> phiAnnotations,
    Dictionary<string, Func<Zen<RouteEnvironment>, Zen<bool>>> psiAnnotations,
    Dictionary<string, Func<Zen<RouteEnvironment>, Zen<bool>>> livenessProperties,
    ISymbolic[] symbolics) : base(
    digraph, transferFunctions, mergeFunction, initialValues,
    digraph.MapEdges(e => witnessTime[e.Item1] < witnessTime[e.Item2]),
    phiAnnotations, psiAnnotations, livenessProperties,
    symbolics)
  {
  }
  ...
  /// <returns></returns>
  public static AnglerInternet2Until ReachableInternal(Digraph<string> digraph,
    Dictionary<(string, string), RouteEnvironmentTransfer> transferFunctions)
  {
    ...
    var monolithicProperties = digraph.MapNodes(n =>
      Internet2Nodes.AsNodes.Contains(n)
    ...
    var witnessTime = digraph.MapNodes<Zen<BigInteger>>(n =>
      Internet2Nodes.AsNodes.Contains(n)
      ? Zen.If<BigInteger>(
        internalRoutes[n].Value.GetResultValue(),
        new BigInteger(0), new BigInteger(1))
      : new BigInteger(0));
    var phiAnnotations = digraph.MapNodes<Func<Zen<RouteEnvironment>, Zen<bool>>>(n => r => Zen.True());
    var psiAnnotations = digraph.MapNodes<Func<Zen<RouteEnvironment>, Zen<bool>>>(n => monolithicProperties[n]);
    ...
    var symbolics = internalRoutes.Values.Cast<ISymbolic>().ToArray();

    return new AnglerInternet2Until(digraph, transferFunctions,
      (r1, r2) => RouteEnvironmentExtensions.MinOptionalForPrefix(r1, r2, InternalPrefix), initialRoutes,
      witnessTime, phiAnnotations, psiAnnotations,
  ...
  /// <param name="transferFunctions"></param>
  /// <returns></returns>
  public static AnglerInternet2Until Reachable(Digraph<string> digraph, string destination,
    ...
    Dictionary<(string, string), RouteEnvironmentTransfer> transferFunctions)
  {
    ...
    var witnessTime = digraph.MapNodes<Zen<BigInteger>>(n =>
      Internet2Nodes.AsNodes.Contains(n)
      ? Zen.If<BigInteger>(
        ExternalNeighborHasRoute(digraph, n, externalRoutes),
        new BigInteger(1), new BigInteger(2))
      : new BigInteger(0));
    var phiAnnotations = digraph.MapNodes<Func<Zen<RouteEnvironment>, Zen<bool>>>(n =>
      Internet2Nodes.AsNodes.Contains(n)
      ? r => Zen.Implies(r.GetResultValue(), Zen.And(r.GetPrefix() == destinationPrefix.Value, NoBadAs(r)))
      : r => Zen.True());
    var psiAnnotations = digraph.MapNodes<Func<Zen<RouteEnvironment>, Zen<bool>>>(n =>
      Internet2Nodes.AsNodes.Contains(n)
      ? r => r.HasPrefixRoute(destinationPrefix.Value)
      : r => externalRoutes.TryGetValue(n, out var externalRoute)
    ...
    //     safety);
    // });
    var symbolics = externalRoutes.Values.Cast<ISymbolic>()
      // .Concat(symbolicTimes)
      .Append(destinationPrefix).ToArray();
    return new AnglerInternet2Until(digraph, transferFunctions,
      (r1, r2) => RouteEnvironmentExtensions.MinOptionalForPrefix(r1, r2, destinationPrefix.Value),
      initialRoutes,
      witnessTime, phiAnnotations, psiAnnotations, monolithicProperties, symbolics);
  }
  ...
  /// <param name="transferFunctions"></param>
  /// <returns></returns>
  public static AnglerInternet2Until ReachableSymbolicPrefix(Digraph<string> digraph,
    ...
    Dictionary<(string, string), RouteEnvironmentTransfer> transferFunctions)
  {
    ...
    var witnessTime = digraph.MapNodes<Zen<BigInteger>>(n =>
      ...
      ? Zen.If<BigInteger>(
        ...
        new BigInteger(2))
      : new BigInteger(0));
    var phiAnnotations = digraph.MapNodes<Func<Zen<RouteEnvironment>, Zen<bool>>>(n => r =>
      Zen.Implies(r.GetResultValue(), Zen.And(r.GetPrefix() == destinationPrefix.Value, NoBadAs(r))));
    var psiAnnotations = digraph.MapNodes<Func<Zen<RouteEnvironment>, Zen<bool>>>(n =>
      Internet2Nodes.AsNodes.Contains(n)
      ? monolithicProperties[n]
