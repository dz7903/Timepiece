...
public class RouteEnvironment
{
  ...
  [JsonConstructor]
  public RouteEnvironment(Ipv4Prefix prefix, uint weight, uint lp, CSet<uint> asSet, BigInteger asPathLength,
    ...
    foreach (var property in properties)
    {
      ...
      switch (property.PropertyType.Name)
      {
        case "CSet":
          ...
          break;
        case "Set":
          var value1 = (Set<string>?) property.GetValue(this);
          if (value1 is null)
            propertiesBuilder.Append($"{value1}");
          else
            propertiesBuilder.AppendJoin(", ", value1.Values.Values.Keys);
          break;
        default:
...
public static class RouteEnvironmentExtensions
{
  ...
  public static Zen<RouteEnvironment> AddAsSet(this Zen<RouteEnvironment> route, uint asnum) =>
    route.WithAsSet(route.GetAsSet().Add(asnum));
